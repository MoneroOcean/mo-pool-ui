...
test.describe("setup, settings, uptime, and copy", { concurrency: false }, () => {
  test("setup output and ports mapping include required miners and ports", () => {
    ...
    assert.match(setupPlanWithPorts({ profile: "multi-miner", gpu: "intel" }).downloadCommand, /mo-miner-v\.\*-lin\\\.tgz/);
    assert.match(setupPlanWithPorts({ profile: "multi-miner", gpu: "intel" }).tlsRunCommand, /MO_MINER='\.\/mo-miner\/mo-miner'/);
    ...
    assert.match(setupPlanWithPorts({ profile: "multi-miner", gpu: "intel" }).tlsRunCommand, /--c29="\$MO_MINER mine 127\.0\.0\.1:3333 \$WALLET x --new\.algo_param\.c29 '\{\\"dev\\":\\"gpu1\*1\\",\\"perf\\":1\}'"/);
    ...
    assert.match(setupPlanWithPorts({ profile: "multi-miner", os: "windows", gpu: "intel" }).downloadCommand, /mo-miner-v\.\*-win\\\.zip/);
    assert.match(setupPlanWithPorts({ profile: "multi-miner", os: "windows", gpu: "intel" }).tlsRunCommand, /\$Mominer="\.\\mo-miner\.cmd"/);
    assert.match(setupPlanWithPorts({ profile: "multi-miner", os: "windows", gpu: "intel" }).tlsRunCommand, /\$MominerJson='\{\\"dev\\":\\"gpu1\*1\\",\\"perf\\":1\}'/);
    assert.match(setupPlanWithPorts({ profile: "multi-miner", os: "windows", gpu: "intel" }).tlsRunCommand, /--c29="\$Mominer mine 127\.0\.0\.1:3333 \$Wallet x --new\.algo_param\.c29 '\$MominerJson'"/);
