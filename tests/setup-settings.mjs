...
test.describe("setup, settings, uptime, and copy", { concurrency: false }, () => {
  test("setup output and ports mapping include required miners and ports", () => {
    ...
    assert.match(setupPlanWithPorts({ profile: "srb-gpu", gpu: "intel", algo: "c29" }).plainRunCommand, /\.\/mom mine gulf\.moneroocean\.stream:10002 .*--new\.algo_param\.c29 '\{"dev":"gpu1\*1"\}'/);
    assert.match(setupPlanWithPorts({ profile: "srb-gpu", os: "windows", gpu: "intel", algo: "c29" }).plainRunCommand, /^\.\\mom\.cmd mine gulf\.moneroocean\.stream:10002 .*--new\.algo_param\.c29 '\{\\"dev\\":\\"gpu1\*1\\"\}'/);
    ...
    assert.match(setupPlanWithPorts({ profile: "multi-miner", gpu: "intel" }).tlsRunCommand, /--c29="\$MOM mine 127\.0\.0\.1:3333 \$WALLET x --new\.algo_param\.c29 '\{\\"dev\\":\\"gpu1\*1\\",\\"perf\\":1\}'"/);
    ...
    assert.match(setupPlanWithPorts({ profile: "multi-miner", os: "windows", gpu: "intel" }).tlsRunCommand, /\$MomJson='\{\\"dev\\":\\"gpu1\*1\\",\\"perf\\":1\}'/);
